...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        if all_losses_below_threshold:
            ...
            model = GRU(batch_size, 1, hidden_size, 1, sparsity, snapLevel, seq_length, type, rank, device)
            input_seq, target_seq = generate_copy_sequence(seq_length, input_size, batch_size)
            jt = torch.zeros((model.hidden_size, model.params.size()[0] * model.params.size()[1]))
