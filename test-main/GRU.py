...
class GRU(nn.Module):
    ...
    def snap1(self, output, targets, hidden, old_hidden, jt, learning_rate):
        ...
        d_loss_d_hidden = torch.autograd.grad(loss, hidden, retain_graph=True)[0]
        Dt = torch.zeros((self.hidden_size, self.hidden_size)).to(self.device)
        It = torch.zeros((self.hidden_size, self.params.size()[0], self.params.size()[1])).to(self.device)
        updated_jt = torch.zeros((self.hidden_size, self.params.size()[0] * self.params.size()[1])).to(self.device)
    ...
    def rtrl(self, output, targets, hidden, old_hidden, jt, learning_rate):
        ...
        It = It.view(self.hidden_size, -1)
        Dt_Jt = torch.matmul(Dt, jt)
        Dt_Jt = Dt_Jt.to(self.device)
        updated_jt = It + Dt_Jt
        updated_jt = updated_jt.to(self.device)
